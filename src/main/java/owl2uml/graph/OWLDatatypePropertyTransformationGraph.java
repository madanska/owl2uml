...
/**
 * Extension of the TransformationGraph, specialized for owlDatatypePropertyGraphCell.
 * It includes always one owlDatatypePropertyGraphCell.
 ...
 * @version June 2006
 */
public class OWLDatatypePropertyTransformationGraph extends ATransformationGraph {
	private static final long serialVersionUID = 1L;
	
	/**
	 * Constructor 
	 */
	public OWLDatatypePropertyTransformationGraph(GraphModel model, String tabTitle, JFrame frame) {
		super(model, tabTitle, frame);
		owlGraphCell = new OWLDatatypePropertyGraphCell(this,100,100);
		getGraphLayoutCache().insert(owlGraphCell);
	}

	/**
	 * Saves the transformation configuration. After adding <owlDatatypePropertyTab> tag, adds the <tabTitle> and  
	 * delegates the saving to OWLDatatypePropertyTab, UMLClassGraphCells and UMLAssociationGraphCells.  
	 */
	public void saveRulesConfiguration(PrintStream printStream) {
		tracer.debug("Saving owlDatatypeProperty transformation configuration");
		printStream.println("	<owlDatatypePropertyTab>");
	...
	/**
	 * Loads the transformation configuration. Reads the <OWLDatatypeProperty>, <UMLClass> and <UMLAssociation> tags 
	 * and delegates loading to them.  
	 */
	public void loadRulesConfiguration(Element tabNode) {
		tracer.debug("Loading owlDatatypeProperty transformation configuration");
		NodeList owlDatatypePropertyNodes = tabNode.getElementsByTagName("OWLDatatypeProperty");
		Element owlDatatypePropertyNode = (Element) owlDatatypePropertyNodes.item(0);
		owlGraphCell.loadRulesConfiguration(owlDatatypePropertyNode);
	...
	/**
	 * If mouse is right clicked on the OWLDatatypePropertyGraphCell, UMLClassGraphCell or UMLAssociationGraphCell on the graph, 
	...
	protected void rightButtonSingleClicked(MouseEvent arg0) {
		...
		} else if(selectedCell.getClass().getName().equals(OWLDatatypePropertyGraphCell.class.getName())) {
			tracer.debug("rightButtonSingleClicked on OWL DatatypeProperty");
			OWLDatatypePropertyGraphCell owlDatatypePropertyCell= (OWLDatatypePropertyGraphCell) getFirstCellForLocation(arg0.getX(), arg0.getY());
			owlDatatypePropertyCell.openPopupMenu(arg0.getX(), arg0.getY());
	...
	public OWLDatatypePropertyGraphCell getOWLGraphCell() {
		return (OWLDatatypePropertyGraphCell) owlGraphCell;
	}
